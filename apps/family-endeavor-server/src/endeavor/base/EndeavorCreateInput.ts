/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  IsDate,
  ValidateNested,
} from "class-validator";
import { Type } from "class-transformer";
import { RewardWhereUniqueInput } from "../../reward/base/RewardWhereUniqueInput";
import { ProgressCreateNestedManyWithoutEndeavorsInput } from "./ProgressCreateNestedManyWithoutEndeavorsInput";
import { EndeavorProgressCreateNestedManyWithoutEndeavorsInput } from "./EndeavorProgressCreateNestedManyWithoutEndeavorsInput";

@InputType()
class EndeavorCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  description?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
  })
  @IsDate()
  @Type(() => Date)
  @IsOptional()
  @Field(() => Date, {
    nullable: true,
  })
  dueDate?: Date | null;

  @ApiProperty({
    required: false,
    type: () => RewardWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => RewardWhereUniqueInput)
  @IsOptional()
  @Field(() => RewardWhereUniqueInput, {
    nullable: true,
  })
  reward?: RewardWhereUniqueInput | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  assignedTo?: string | null;

  @ApiProperty({
    required: false,
    type: () => ProgressCreateNestedManyWithoutEndeavorsInput,
  })
  @ValidateNested()
  @Type(() => ProgressCreateNestedManyWithoutEndeavorsInput)
  @IsOptional()
  @Field(() => ProgressCreateNestedManyWithoutEndeavorsInput, {
    nullable: true,
  })
  progresses?: ProgressCreateNestedManyWithoutEndeavorsInput;

  @ApiProperty({
    required: false,
    type: () => EndeavorProgressCreateNestedManyWithoutEndeavorsInput,
  })
  @ValidateNested()
  @Type(() => EndeavorProgressCreateNestedManyWithoutEndeavorsInput)
  @IsOptional()
  @Field(() => EndeavorProgressCreateNestedManyWithoutEndeavorsInput, {
    nullable: true,
  })
  endeavorProgresses?: EndeavorProgressCreateNestedManyWithoutEndeavorsInput;
}

export { EndeavorCreateInput as EndeavorCreateInput };
