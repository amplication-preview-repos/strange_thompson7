...
describe("PrizeProgress", () => {
  ...
  test("GET /prizeProgresses/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/prizeProgresses"}/${nonExistingId}`)
  ...
  test("GET /prizeProgresses/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/prizeProgresses"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
